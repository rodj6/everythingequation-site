...
\usepackage{booktabs,longtable,tabularx,array,enumitem,xcolor,xurl}
\usepackage[colorlinks=true,linkcolor=blue!40!black,citecolor=blue!40!black,urlcolor=blue!40!black]{hyperref}
\usepackage{bookmark,fancyhdr}
\pagestyle{fancy}\fancyhf{}\fancyhead[L]{\small A deterministic pilot medium and its Bell limit}\fancyfoot[C]{\thepage}
...
\hypersetup{pdftitle={A Deterministic Pilot Medium: Bell Path Selection and Autonomous Material Records},pdfauthor={Research programme of Jeremy Rodgers},pdfsubject={Version 2; constitutive theory and controlled physical-time Bell-path limit}}
\title{%
  \bfseries A Deterministic Pilot Medium:\\
  Bell Path Selection and Autonomous Material Records\\[6mm]
  {\large A constitutive completion with a controlled physical-time limit}
}
...
\section{Event-law selection and the scope of the result}
For a complete ordinary configuration $X$ and fixed orthogonal configuration basis, the target is
\begin{equation}
...
\item The normalized canonical field and primitive bond connections obey the action in Section~\ref{sec:source}. All ordinary forces, including coherent feedback, enter its numerical Hermitian matrix $H$. An ordinary controller is another factor in that same matrix.
...
\section{Canonical edge ownership and conservative export}\label{sec:source}
Use the common action
\begin{align}
 S&=\int\left[\frac{i\hbar}{2}(\Psi^\dagger\dot\Psi-\dot\Psi^\dagger\Psi)
       +\hbar\sum_e\Pi_e\dot\chi_e-h(\Psi,\chi,t)\right]\dd t,\\
 h&=\sum_r\Psi_r^*H_{rr}\Psi_r+
    \sum_{e=(r,q)}\left(e^{i\chi_e}\Psi_q^*H_{qr}\Psi_r+\mathrm{c.c.}\right).
 \label{eq:action}
\end{align}
Block sectors can replace scalars throughout with their inner products. Passive connection ownership means that $h$ has no $\Pi$ dependence. At $\chi(0)=0$, Hamilton's equations give
\begin{equation}
...
\begin{proposition}[Primitive bond ownership]
...
\end{proposition}
\begin{proof}
A binary cross term is $\Psi_q^*T_e(\chi_e)\Psi_r+\mathrm{c.c.}$.
Covariance gives $T_e(\chi+\delta)=e^{i\delta}T_e(\chi)$, so $T_e(\chi)=e^{i\chi}H_{qr}$. Differentiate the action. Vertex-diagonal terms have zero bond torque. Every finite Hermitian $H$ supplies a realization.
\end{proof}
...
Allocate a fixed channel for every potential pair $(e,b,a)$ of packet slot and carrier, with frequency parameter $r_{eba}=\kappa_e\mu_N/N$. Allocate a channel for every unordered pair of slots on edge $e$, with parameter $a_e>0$. A contact tests only the current species and carrier vertex. An eligible service implements \eqref{eq:service}; opposite slot species recombine and retain their signs and identities in the outgoing product; other contacts are null. The total candidate frequency is
\begin{equation}
...
The device's initial state may itself be random: the same conditional
law is used in both comparisons, independently of the gas, and is
averaged after applying the deterministic map.
...
\begin{theorem}[Finite-gas marked-history bound]\label{thm:gas}
...
For the Poisson contact comparison, every eligible packet--carrier pair has derived rate $\kappa_e\mu_N/N$, and every opposite slot pair has derived rate $a_e$. Let $P_e^\pm$ be species counts, $m_e^{\rm mix}=\min(P_e^+,P_e^-)$, and $A_e,D_e$ the recombination and service counts. Starting from empty packet stock, exactly
\begin{equation}
...
\begin{theorem}[Full reaction-state recombination comparison]\label{thm:annihilation}
...
\begin{equation}
\TV\bigl(\Law(S^{a}_{[0,T]}),\Law(S^{\rm lift}_{[0,T]})\bigr)
...
\section{The full Bell limit and its conditional history}\label{sec:mainlimit}
Define $\epsilon_{\rm kin}(N,T)$ to be the signed-queue tagged-path bound \eqref{kin:pathbound}, proved in Theorem~\ref{kin:path} of Appendix~\ref{app:kinetic}, with the cutoff choices made there. Under that appendix's hypotheses it tends to zero for fixed finite graph and programme if
\begin{equation}
 \mu_N\longrightarrow\infty,\qquad \mu_N/N\longrightarrow0,
 \qquad \E\|x^N(0)-w(0)\|_1\longrightarrow0.
 \label{eq:kineticscale}
\end{equation}
...
\begin{theorem}[Deterministic microscopic selection of the Bell path]\label{thm:main}
Fix the following data and hypotheses of the pilot-medium theory P1--P4:
\begin{enumerate}[label=(\roman*)]
\item A finite ordinary configuration graph $V$, a declared finite
sector/reference convention, a finite physical horizon $[0,T]$, and a
fixed bounded piecewise $C^1$ admitted Hermitian programme $H(t)$.
The resulting coherent currents $J_e$ have bounded variation on the
horizon. The graph, programme and all retained ordinary systems are
fixed as $N$ grows. Every ordinary control and feedback interaction is
included in this programme.
\item Fixed scalar response coefficients
$0<\kappa_-\le\kappa_e\le\kappa_+<\infty$, the complete binary
reaction list, and the conservative exporter of
Section~\ref{sec:source}, with its uniform $O(N^{-1})$ discrepancy
and per-edge $O(N)$ birth budgets $B_e$. Packet stock is initially
empty. Initial exporter residues are zero, or uniformly bounded as
allowed by \eqref{eq:export} and Appendix~\ref{app:kinetic}.
\item Conditional on the fixed initial coherent preparation, the
predesignated tag has a fixed law $\nu$ with
$\nu_r\le Cw_r(0)$ for every $r$, where $C<\infty$ is independent
of $N$. Its $N-1$ companion carriers are independent samples from
$w(0)$, independent of the tag. The census is consequently calibrated
in expectation as in \eqref{eq:kineticscale}. More generally the
same conclusion applies to an admitted calibrated census with this
tag law and $\E\|x^N(0)-w(0)\|_1\to0$, under the independent
comparison-clock initialization of Appendix~\ref{app:kinetic}.
\item The finite beam is initialized independently of the entire
reaction device, conditional on the declared coherent preparation,
with the longitudinal and transverse product law of Section~4.
For $R_N>0$, $M_N>R_NT$. All packet slots, exporter fuel/blank
cells, contact receivers and retained reaction-product banks are
supplied for the stated budgets. The physical finite device and its
Poisson comparison use the same causal map, tie order and virtual
overflow convention of Theorem~\ref{thm:gas}.
\end{enumerate}
On the common path space $D([0,T],V)$, with its coordinate
$\sigma$-field, every finite realization satisfying these hypotheses obeys
\begin{equation}
 \TV\bigl(\Law(Q^N_{[0,T]}),\mathbb P^B_{H,\nu,[0,T]}\bigr)
 \le \Delta_N:=\frac{(R_NT)^2}{M_N}
       +\sum_e\frac{\kappa_e\mu_NB_e}{2a_e}
       +\epsilon_{\rm kin}(N,T).
 \label{eq:mainerror}
\end{equation}
In general the bound tends to zero when
\eqref{eq:kineticscale} holds and both
$(R_NT)^2/M_N\to0$ and
$\sum_e\kappa_e\mu_NB_e/(2a_e)\to0$.
In fixed physical units, the concrete choice $B_e=O(N)$,
$\mu_N=N^{1/2}$, $a_e=N^2$, and $M_N=N^{10}$ gives
$R_N=O(N^4)$, gas error $O(N^{-2})$, recombination error
$O(N^{-1/2})$, and $\Delta_N\to0$. Each resource is finite at
each $N$; the capacities and $M_N>R_NT$ condition are imposed
before the run. No change of time accompanies this limit. The target
is the nonexplosive minimal Bell process \eqref{eq:target},
initialized at $\nu$, with its complete natural conditional timing law.
\end{theorem}
\begin{proof}
Condition first on the declared coherent preparation. The initialized
causal source, exporter and contact device is independent of the gas;
Theorem~\ref{thm:gas} therefore compares its complete reaction output
under the spatial beam and the Poisson contacts. In the latter model,
predictable eligibility gives precisely the per-slot mass-action
reactions of Theorem~\ref{thm:annihilation}. That theorem couples
the complete reaction states until the first discrepant matched-packet
service, using the stopped physical compensator and empty-stock
budget. Its lifted reference then projects exactly to the signed queue
of Theorems~\ref{kin:tracking} and \ref{kin:path}.

The initial domination, expected census calibration, bounded-variation
currents and fixed exporter/response bounds are the hypotheses of
those kinetic results. Their expectations include the initial census;
they do not require a deterministic realization of the empirical
populations. Apply the tagged-path bound on $D([0,T],V)$ and use
the triangle inequality after the preceding common projections. This
gives \eqref{eq:mainerror}. The growth estimates follow from
\eqref{eq:candidate}; $B_e=O(N)$ gives $R_N=O(N^4)$ under the
displayed scales. Lemma~\ref{kin:existence} supplies nonexplosion
and domination through nodal boundaries, and the nodal coupling in
Theorem~\ref{kin:path} identifies the entire limiting law.
\end{proof}
...
The theorem concerns $Q$, which already contains every ordinary source, archive, receiver, controller and reference coordinate of the declared experiment. It does not identify the Bell law with the full microstate \eqref{eq:microstate}. All pilot histories remain physically present under their different force law. A later programme that reads or returns pilot products is not covered by silently tracing them out. P3 rules out an extra ordinary read force; changes of pilot dynamics or resource recirculation require a new estimate. Coarse records contract \eqref{eq:mainerror}, but their jump rates need not be the positive part of a sum of fine currents.

\section{An autonomous material programme in the same canonical field}
\label{mat:section}
...
\cite{Feynman1986,Christandl2004}; all facts used here are proved below.

\subsection{Exact autonomous propagation}
...
the identity on $R$.  Proposition~\ref{mat:basisprep} also permits an
explicit earlier preparation stage involving $R$.  Set
\begin{equation}\label{mat:prefix}
...
\begin{equation}\label{mat:feynman}
...
\begin{theorem}[Exact finite-clock programme]\label{mat:clock}
Starting with $|0\rangle\otimes\psi$, $\|\psi\|=1$, the field under
\eqref{mat:feynman} is
\begin{align}\label{mat:wave}
...
Its normalized symmetric coefficients prove \eqref{mat:wave} and
\eqref{mat:time}.  The spectrum of the qubit sum lies in
...
\begin{equation}\label{mat:weights}
...
directly from \eqref{mat:weights}.
\end{proof}
...
\subsection{A faithful archive is created at a monomial clock cut}

A unitary is \emph{monomial} in the complete material basis when
\begin{equation}\label{mat:monomial}
 (U_m)_{yx}=e^{i\vartheta_x}\,1_{\{y=\pi(x)\}}
\end{equation}
for a permutation $\pi$.  Reversible copies and SWAPs are examples.

\begin{theorem}[Historical record at a monomial cut]\label{mat:copy}
For the Bell process of \eqref{mat:feynman} in initial equilibrium, a
...
\begin{equation}\label{mat:finecurrent}
 J_{(m+1,y),(m,x)}(t)=2\Omega c_ma_{m+1}a_m
   \operatorname{Re}\left[(\xi_{m+1})_y^*(U_m)_{yx}(\xi_m)_x\right].
\end{equation}
For \eqref{mat:monomial}, the nonzero real factor is exactly
...
\label{mat:finewarning}
For a general unitary $U_m$, the real factor in
\eqref{mat:finecurrent} can be negative.  The net clock flux can be
...
Theorem~\ref{mat:copy} uses the monomial hypothesis essentially.
In particular, a later archive does not record every transient
excursion of an earlier nonmonomial resource gate.
\end{remark}

\section{An explicit retained-resource measurement and continuation}
\label{mat:resources}
...
For $0<\eta<1$, define
\begin{align}\label{mat:generators}
...
\begin{equation}\label{mat:rotations}
...
\begin{equation}\label{mat:resourcevector}
...
ready state is
\begin{equation}\label{mat:ready}
...
\eqref{mat:feynman} with the gates \eqref{mat:circuit}.  Tensor and
...
$B_W$.  Apply Theorem~\ref{mat:copy} at each cut.  The final cut has
...
Choose
\begin{equation}\label{mat:input}
...
The resource weights in \eqref{mat:resourcevector} are respectively
...
Let $\mathsf N$ be the archive value $A=0$.  Tracing the retained resource
factors only for this calculation gives the null subchannel
\begin{equation}\label{mat:nullmap}
...
\begin{equation}\label{mat:nullnumbers}
...
Adding the two matrices gives \eqref{mat:reference}.
\end{proof}
...
\begin{align}\label{mat:capturednumbers}
...
Prepend to the nine gates \eqref{mat:circuit} the two gates
\begin{align}\label{mat:prepgates}
 U_{\rm prep}
...
for \eqref{mat:input}, with all remaining material resources still ready.
...
\eqref{mat:nullnumbers}--\eqref{mat:capturednumbers}, are unchanged.
\end{proposition}
\begin{proof}
The sign convention in \eqref{mat:prepgates} gives
\[
 R_y(\alpha)|0\rangle=\sqrt{2/3}|0\rangle+
                                \sqrt{1/3}|1\rangle.
\]
The following controlled Hadamard therefore maps $|0,0_R\rangle$
exactly to \eqref{mat:input}.  The clock proof applies to these eleven
...
Theorem~\ref{mat:copy} gives its unique forward crossing and its
...
\eqref{mat:feynman} obeys
\[
 \TV\bigl(\Law(Q^{\rm pilot}_{[0,T]}),
            \Law(Q^{\rm Bell}_{[0,T]})\bigr)\le\delta.
\]
...
Proposition~\ref{mat:resourcehistory} fails is at most $\delta$;
the probabilities $\Pr(\mathsf N)$, $\Pr(\mathsf N,X+)$ and the captured joint outcomes
in \eqref{mat:capturednumbers} have that same error bound.  The
...
\end{proof}
This is a joint bound on the common path; it has no factor equal to
the number of records or receiving systems.  A conditional comparison
requires positive branch probability under both laws.  To be explicit,
let $P$ be the ideal law, $Q$ the finite pilot law and $C$ the conditioning
event, with $p=P(C)>0$ and $q=Q(C)>0$.  Since $|p-q|\le\delta$,
$\delta<p$ suffices to ensure $q>0$.  For any event $F\subseteq C$,
\[
 \left|\frac{P(F)}p-\frac{Q(F)}q\right|
 \le\frac{|P(F)-Q(F)|}{p}+\frac{Q(F)|p-q|}{pq}
 \le\frac{2\delta}{p}.
\]
Taking the supremum gives conditional TV error at most
$\min\{1,2\delta/p\}$.  For the null branch above, $p=3/4$;
thus $\delta<3/4$ suffices and the conditional error is at most
$\min\{1,8\delta/3\}$.  No uniform precision is asserted on
arbitrarily rare branches.  Nor does a classical-path TV estimate alone compare an
...
Proposition~\ref{mat:levels} provides the indicated input-uniform
...
\section{Adversarial tests and the surviving alternatives}\label{sec:rivals}
\subsection{A stationary coherent cycle tests individual edge ownership}
Take three vertices with $w_r=1/3$, $\Psi=(1,1,1)^T/\sqrt3$, and
\[
 H=\frac{3\hbar j}{2}
 \begin{pmatrix}0&-i&i\\i&0&-i\\-i&i&0\end{pmatrix},\qquad j>0.
\]
Then $H\Psi=0$ but $J_{21}=J_{32}=J_{13}=j$. The canonical exporters therefore produce clockwise packets even though every vertex population is stationary. Theorem~\ref{thm:main} gives a clockwise Bell cycle at rate $3j$, with Poisson$(3jT)$ jump count and its full continuous event times. A generator which simply keeps the configuration fixed has the same one-time equilibrium and zero divergence, yet differs in path law by $1-e^{-3jT}$. It is excluded by individual primitive bond ownership, not by the continuity equation alone.
...
\subsection{The finite-recombination rival really has dark traffic}
...
\begin{equation}
 ...
 \label{eq:darkrival}
\end{equation}
The first event is a race of the two derived contact mechanisms. Recombination first removes both packets; service first already makes the path nonconstant. The instantaneous net queue has $Z=0$ and never moves, so \eqref{eq:darkrival} is its exact carrier-path distance from this finite reaction experiment. A mixed stock can occur after opposite exports before recombination completes. This local test does not replace the empty-queue initialization of Theorem~\ref{thm:main}. It shows that the new mechanism has finite-speed predictions and is not a renamed Jordan decomposition. If recombination is absent or too slow, surplus survives.

\subsection{The spatial ensemble selects timing beyond mean flux}
...
\subsection{Recombination does not conceal a readable pilot ledger}
...
\end{proposition}
\begin{proof}
Direct two-state wave evolution gives event probabilities $\sin^2\tau,0,\sin\tau\cos\tau$. For the first and third preparations the current is forward throughout the interval, so there is at most one forward event and its probability is the loss of origin weight. For the second it is backward. If a positive effect has error at most $\varepsilon$ on all three, its trace is at most $\sin^2\tau+2\varepsilon$, whereas its expectation in the third vector is at least $\sin\tau\cos\tau-\varepsilon$. A positive expectation cannot exceed the trace. Rearrangement gives \eqref{eq:nativegap}.
\end{proof}
...
\subsection{Fine and coarse paths remain distinct}
...
\section{Integration with the monograph and final scope}
...
Signed queue & Instant opposite cancellation was supplied & Replaced by actual two-species coexistence and finite recombination; new full-state error \eqref{eq:annerror}.\\
Reaction clocks & Additive Markov pair generator was supplied & Replaced by deterministic finite flight and independent spatial preparation; new complete-history error \eqref{eq:gaserror}.\\
...
Thus the original unrestricted programme is not proved inevitable. A precisely stated replacement constitution now supplies a complete event-selection and measurement-chain realization with quantitative path errors. The massive continuous-configuration completion \cite{MassiveCompletion} remains a separate alternative; its operational agreement must not be mislabeled as this Bell-path derivation.

\subsection{Proofs and literature provenance}
The spin-clock identification is the finite engineered-chain mechanism of Christandl et al.\ \cite{Christandl2004}; the clock carrying unitary gates is the Feynman Hamiltonian construction \cite{Feynman1986}. We verify their specific matrices and solutions here instead of importing an unspecified computation theorem. Neither source supplies a Bell event-selection law. Bell-process existence is the finite-graph form of the framework in D\"urr et al.\ \cite{Bell2005}, with the needed node proof reproduced in the appendix. All gas, recombination and archive-path estimates used for closure are proved in this manuscript.
...
\section{Self-contained kinetic implication used in the limit}\label{app:kinetic}
This appendix reproduces the monograph's established signed-queue argument, with notation fixed here. It is not counted as a new selection of that model. The new physical replacements are Theorems~\ref{thm:gas} and \ref{thm:annihilation}.

Let $D=|V|$, $L=\sum_e L_e$, $0<\kappa_-\le\kappa_e\le\kappa_+$, and use the oriented incidence matrix already defined. In the signed-queue comparison, $Z_e$ has only its net species, and each packet--eligible-carrier pair reacts at rate $\kappa_e\mu_N/N$. Put
\begin{align}
 x_r&=N^{-1}\#\{a:X_a=r\},\qquad m_e=\mu_N Z_e/N,\\
 \Phi_e^{N,+}&=\kappa_e x_r[m_e]_+,\qquad
 \Phi_e^{N,-}=\kappa_e x_q[-m_e]_+ .\label{kin:flux}
\end{align}
These are actual normalized bulk reaction intensities, not sample counting measures. A fixed carrier at the relevant origin has rate $\kappa_e[\pm m_e]_+$. Thus its conditional rate is $\Phi_{qr}^N/x_r$ when it is at $r$. This denominator follows from counting its possible packet partners among $Nx_r$ carriers. Every service consumes one packet, hence total service count is at most $NL+O(1)$.
Write $A_e^N:=k_e/N$ for the normalized cumulative signed export.
The coherent nodal estimate is
\begin{equation}
 |J_{qr}|\le (2/\hbar)\|H_{qr}\|\sqrt{w_qw_r} .\label{kin:nodal}
\end{equation}
All graph-dependent constants below are finite at fixed graph and horizon. Write $C_B$ for an incidence norm, $C_H,C_0,C_G$ for the corresponding fixed coherent current bounds, and $c_0$ for the uniform exporter error constant. Coefficients $\kappa_e$ are fixed; arbitrarily varying kinetic coefficients are not in this theorem.
Assume empty initial queues and calibrated initial populations with
$\mathbb E\|x^N(0)-w(0)\|_1\to0$. Deterministic census preparation is a
special case. Randomized census preparation is also allowed; conditional
on the initial field, it is independent of future comparison reaction
clocks. Let $z^N=Z/N$ and
...
\end{theorem}
\begin{proof}
We give the tracking and low-population steps separately. The same
pathwise inventory and variation estimates hold after conditioning on
the initial census; expectations below average that census as well as
reaction clocks. Fix a cutoff
$0<\delta\le1$ and, on each edge, run a companion signed queue from the same
...
\[
 ...
 +\frac{J_*}{a_0^2}[\Var(a_+)+\Var(a_-)].
\]
Contraction between jumps of $f$ and summation of its jumps imply
\[
 \int_0^T|y_J-f|dt\le\frac{|f(0)|+\Var(f)}{\mu_Na_0}.
\]
...
\subsection{The complete tagged path and the nodal boundary}
Total variation means $\TV(P,Q)=\sup_A|P(A)-Q(A)|$ on the measurable
...
For \eqref{eq:target}, the minimal rates
...
\cite{Bell2005}; it is not a selection of that process among all event laws.
\end{proof}
...
Then
\begin{equation}
 \TV\bigl(\Law(X_{a_*}^N),\Law(Q^B)\bigr)\longrightarrow0,
...
$\int_0^1N_\varepsilon d\varepsilon\le\sum_r\Var(w_r)$.
...
\subsection{Initial independent equilibrium and fixed-circuit uniformity}
...
On $[0,2T]$ the spin clock reverses and returns, so at most four level entries per weight give $b(\varepsilon)\le5D\varepsilon+2C_0T\sqrt\varepsilon$. The path theorem still applies across the node at $T$ and all reversed currents. The unitary echo also unwrites the circuit's records; it is a reversal test of the event law, not an archive-protection theorem beyond the first pass. Exact zero-current intervals in the general programme are included in the same estimates. Finite pilot queues can produce delayed events there, but their total path discrepancy is already in \eqref{eq:mainerror}; they are not declared absent at finite resources.

\subsection{An explicit fixed-circuit resource rate}
\label{kin:uniformity-rate}
The qualitative limit in Theorem~\ref{thm:main} uses the preceding
kinetic proof. For the fixed clock circuit, its constants can also be
controlled uniformly over unknown inputs. The following refinement,
retained from the corrected monograph~\cite{Monograph}, records the
cutoff dependence explicitly and supplies its complete proof. It is not
a new stochastic law or a rate for growing circuits. The clock-weight
shape was proved in Proposition~\ref{mat:levels}.

\begin{proposition}[Uniform fixed-circuit kinetic rate]\label{kin:rate}
...
\begin{equation}\label{kin:explicit-rate}
...
of Theorem~\ref{thm:main}, the full pilot path error is also $O(N^{-1/70})$.
\end{proposition}
\begin{proof}
Write $D=|V|$, $m=|E|$, $H_* =\|H_F\|$, and let $\Var$ denote total
...
\[
 ...
 \sum_e\Var(J_e)\le 4mH_*^2T/\hbar^2.
\]
...
$\Var(f)=O(\delta^{-2})$ and $|f(0)|=O(\delta^{-1})$.
...
\begin{align}\label{kin:cutoff-rates}
...
term of \eqref{kin:cutoff-rates} is $N^{-1/14}$, its square-root noise
...
For the clock family \eqref{mat:weights}, each fine weight is a
...
$O(N^{-3/70})$.  This proves \eqref{kin:explicit-rate}.
...
$\mathbb E\|x^N(0)-w(0)\|_1\to0$ without a rate does not imply
\eqref{kin:explicit-rate}.  Nor does this estimate apply uniformly to a
growing graph, increasing fine reference, changing Hamiltonian or unbounded
storage time.

\section{A smooth autonomous realization of the finite contact module}
\label{mech:section}
\label{app:mechanical}

This section supplies an explicit Hamiltonian for a finite family of reversible
...
\subsection{Register cells and an explicit permutation compiler}
...
$0<\delta<T$, define
\begin{equation}\label{mech:field}
...
\subsection{Positive kinetic coupling and exact passage time}
...
\label{mech:hamiltonian}
For every finite parameter choice, \eqref{mech:H} is smooth, autonomous,
...
\begin{align}
 ...
 t_{\rm out}=t_{\rm in}+\delta,\label{mech:time}\\
 p(t)&=p_{\rm in},\nonumber\\
 q(t)&=\Gamma_{c,i}\!\left(\frac{t-t_{\rm in}}{\delta}\right)
       +(q_{\rm in}-Q_i)
       +\frac{p_{\rm in}}{\mu}(t-t_{\rm in}).\label{mech:transport}
\end{align}
...
This proves \eqref{mech:time}; inserting \eqref{mech:field} into the
register equation gives \eqref{mech:transport}.
At exit the field vanishes again, so $P_j=k_j=m_bv$.
\end{proof}

\begin{corollary}[Uniform composition through separated contacts]
\label{mech:robust}
Assume \eqref{mech:cells} and
\begin{equation}\label{mech:margin}
...
is at most $r_0+p_*t/\mu$. Inequality \eqref{mech:margin} prevents a first
exit from a track core and closes the bootstrap used in
Lemma~\ref{mech:exactgate}.
\end{proof}

The energy before and after every separated gate is
\begin{equation}\label{mech:energy}
...
\subsection{Present-state logical decoding and path topology}
...
\subsection{Smooth channel plates and the projected history bound}
...
\eqref{mech:margin}. Assume that no other interaction changes the register
...
Then
\begin{equation}\label{mech:TV}
 \TV\!\left(
 \Law(\widehat S^\delta,\widehat\Xi^\delta),
 \Law(S^{\rm P},\Xi)\right)
 \le
...
On the good event, Lemma~\ref{mech:exactgate} and
Corollary~\ref{mech:robust} identify the actual committed circuit with the
...
\label{mech:scope}
Theorem~\ref{mech:history} is a supplementary smooth realization of the
...
The static field \eqref{mech:field} encodes the supplied reaction
...
Chapters 3--4, 6--8 and 35 give the canonical, kinetic and pilot results and their consolidated scope; Appendix D gives the supplementary smooth contact construction. The arguments needed here are reproduced in the present paper.

\bibitem{Bell2005}
D. D\"urr, S. Goldstein, R. Tumulka and N. Zangh\`i,
``Bell-type quantum field theories,''
\emph{Journal of Physics A: Mathematical and General} \textbf{38}, R1--R43 (2005).
\href{https://arxiv.org/abs/quant-ph/0407116}{arXiv:quant-ph/0407116};
\href{https://doi.org/10.1088/0305-4470/38/4/R01}{doi:10.1088/0305-4470/38/4/R01}.
...
The engineered spin-chain matrix and rotation are equations (13)--(15) in the primary paper.
...
A massive continuous-configuration constitutive alternative, cited for comparison; its event law is distinct from the minimal Bell graph path.
