...
- `content/monograph/*.mdx` — one file per chapter (18), appendix (A, B), and
  ...
  (chapter|appendix|bibliography), `number`, `label`, `part`, `order`,
  `description`.
...
- Pages: `src/app/monograph/page.tsx` (hub: abstract, organization, full TOC,
  Book JSON-LD), `src/app/monograph/[slug]/page.tsx` (chapter pages with
  prev/next navigation and Chapter JSON-LD), and
  `src/app/monograph/manifest.json/route.ts` (machine-readable inventory of
  the complete work for AI systems and crawlers).
- Reading helpers live in `src/lib/monograph.ts`; the prebuild scanner
  (`generateMonograph` in `scripts/generate-content-maps.mjs`) writes
  `src/generated/monograph.ts` from the MDX files.
- The chapters are included automatically in `/sitemap.xml`, `/llms.txt`,
  `/graph.json` and `/feed.xml` via `src/lib/machine.ts`.
- MDX pipeline note: `remark-gfm` (added in `next.config.mjs`) is required for
  the monograph's pipe tables. Do not remove it.
- If a new Zenodo VERSION of the monograph is ever published, update the DOI
