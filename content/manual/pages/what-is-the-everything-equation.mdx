...
## What remains after closure — K∞ and the resonance skeleton (CRS*)
...
A law-object here is not a single formula; it is the candidate rule “what counts as the physically admissible evolution” given f and u0.
...
Define ∂ as canonical restriction / normalization of representation.
...
Output: ∂(L) = the canonized weak-solution carrier (a normalized admissible presentation space)

Intuition: ∂ is the step that says: “first decide what counts as a legitimate representation of the dynamics.”
...
Step 2 — Persistence filtering: Δ

Define Δ as the persistence filter: the operation that removes non-stable structure under the domain’s stabilization criterion.
...
taking the weak-solution carrier ∂(L),
...
Intuition: Δ is the step that says: “only those candidates that survive stabilization are law-eligible.”
...
Step 3 — Closure completion: Ω

Define Ω as closure completion / reflective completion under admissible re-expression.
...
In this domain, Ω is precisely the operation that returns the entropy solution as the canonical completion of the admissible class. Concretely, Ω can be realized via any equivalent closure mechanism, for example:

vanishing viscosity completion:
u_eps_t + (f(u_eps))_x = eps * u_eps_xx,
with eps going to zero from above,
where the limit defines the entropy solution;

or the Kruzkov / contraction semigroup completion:
entropy solutions form an L1-contractive semigroup (closure under time composition);
...
Intuition: Ω is the step that says: “complete the admissible survivors into a stable law-object.”
...
and the ∂–Δ–Ω decomposition is minimal and ordered.

Why this toy example transfers universally

This same pattern is what makes the Everything Equation domain-instantiable:

In QFT / RG, ∂ is scheme or gauge normalization, Δ is universality and persistence under coarse-graining, Ω is renormalized completion (consistent effective theory).

In gauge theory, ∂ is gauge equivalence, Δ is anomaly and persistence filtering, Ω is global bundle and consistency completion.

In measurement, ∂ is admissible re-description of the system–observer split, Δ is record formation and decoherence persistence, Ω is closure of the record algebra and the probability rule.

In hard PDE regimes, ∂ canonizes the carrier, Δ is the strict-margin or persistence term, Ω is closure to a continuation or regularity class or an explicit failure-channel classification.
