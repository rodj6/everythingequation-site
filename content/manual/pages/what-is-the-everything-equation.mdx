...
| Tier-0 operator | Role in realization |
|---|---|
| $\partial$ (boundary normalization) | Remove scaling gauge via UV pinning **(E5)** + global scheme fixation |
| $\Delta$ (persistence/records) | CP semigroup $\Phi_t$, positive generator $K$, Fejér operator and budget **(E2)** |
| $\Omega$ (reflective closure) | Dirac-side record demand **(E3)**, feasibility constraint **(E4)**, IR stability window **(E6)** |
| Fixed point $\mathcal{L} = \Omega\Delta\partial(\mathcal{L})$ | Stable coupled feasibility at all scales; variational enforcement yields KKT **(E7)** and saturation equations **(E8)** |
| Reversible face choice | Spectral action dynamics **(E1)** |
